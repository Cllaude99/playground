...
export default function ConfirmStep({ signUpData }: ConfirmStepProps) {
  ...
  return (
    <div className="flex flex-col h-full gap-3">
      ...
      <Input inputType="readOnly" label="이름" value={signUpData.name} />
      <Input inputType="readOnly" label="지역" value={signUpData.location} />
      <Input
        inputType="readOnly"
        label="생년월일"
      ...
      <Input
        label="만나이"
        value={`${getAgeFromBirthdate(signUpData.birthdate)}세`}
        inputType="readOnly"
      />
      {isUnder14(signUpData.birthdate) && (
        <Input
          label="법정대리인 동의 여부"
          value={signUpData.agreement ? '동의' : '미동의'}
          inputType="readOnly"
        />
      )}
      <Input
        inputType="readOnly"
        label="성별"
        value={getGender(signUpData.gender)}
      />
